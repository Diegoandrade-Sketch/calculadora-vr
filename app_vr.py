...
st.markdown("""
    ...
    /* TITULO HARMONIZADO COM MAIS DESCIDA */
    .hero-title {
        color: #262730;
        font-size: 3.2rem; 
        font-weight: 900; 
        margin: 0; 
        padding-top: 50px; /* Desce mais o título para alinhar com o centro da logo */
        line-height: 1; 
        letter-spacing: -1px; 
    ...
    .tooltip .tooltiptext {
        ...
        margin-left: -140px; opacity: 0; transition: opacity 0.3s; font-size: 0.85rem; font-weight: 400;
    }
    .tooltip:hover .tooltiptext { visibility: visible; opacity: 1; }
    </style>
    """, unsafe_allow_html=True)

# --- LOGICA DE DADOS (Dicionários permanecem iguais) ---
...
descricoes_imp = {
    ...
    "Definição de Escopo": "Mapeamento dos processos para garantir que as necessidades sejam atendidas.",
    "Configuração Servidor / PDV Linux": "Preparação técnica do servidor e terminais com sistema Linux estável.",
...
# --- CABEÇALHO DINÂMICO ---
# A Logo sempre aparece, mas o título "PROPOSTA COMERCIAL" some no modo apresentação
head_col1, head_col2 = st.columns([1, 2.5])

with head_col1:
    if os.path.exists("logo_vr.png"):
        st.image("logo_vr.png", width=320)
    else:
...
# 5. Interface de Seleção (Só aparece se NÃO estiver em modo apresentação)
if not modo_apresentacao:
    col1, col2, col3 = st.columns(3)
    # (Logica de inputs omitida para brevidade, mas mantida funcional no seu código)
    # ... código de inputs aqui ...
    # [Mantendo a lógica de cálculo que você já tem]
    dados_imp_final = []
    with col1:
        ...
        for item in imp_sel:
            v_u = itens_imp[item]
            h = st.number_input(f"Hrs: {item}", min_value=0, value=12 if "Treinamento" not in item else 120, key=f"h_{item}")
            t_imp += h * v_u
    ...
    # Recupera dados para exibição no modo apresentação
...
# 6. RESUMO VISUAL (Fica no topo no modo apresentação)
st.markdown("<h2 style='text-align: center; color: #333; font-weight: 800; margin-top: 20px; margin-bottom: 30px;'>DETALHAMENTO DA PROPOSTA</h2>", unsafe_allow_html=True)
...
with res_col1:
    ...
    st.markdown(f'<div class="resumo-card">Investimento Setup<div class="resumo-valor">R$ {t_imp:,.2f}</div><div style="font-size: 1.2rem; font-weight: bold;">{parcelas}x de R$ {t_imp/parcelas:,.2f}</div><ul class="lista-itens">{html_itens}</ul></div>', unsafe_allow_html=True)

with res_col2:
    html_itens = "".join([f"<li><span>{i}</span><span class='item-detalhe'>{q} Lic. x R$ {v:,.2f}</span></li>" for i, q, v in dados_mensal_final])
    st.markdown(f'<div class="resumo-card" style="border-top-color: #2e7d32;">Investimento Mensal<div class="resumo-valor">R$ {t_men_liq:,.2f}</div><div style="font-size: 1.1rem; color: #2e7d32; font-weight: bold;">Desconto: {desc}%</div><ul class="lista-itens">{html_itens}</ul></div>', unsafe_allow_html=True)

with res_col3:
    html_itens = "".join([f"<li><span>{i}</span><span class='item-detalhe'>{q} Qtd. x R$ {v:,.2f}</span></li>" for i, q, v in dados_desp_final])
    st.markdown(f'<div class="resumo-card" style="border-top-color: #1976d2;">Previsão de Despesas<div class="resumo-valor">R$ {t_desp:,.2f}</div><div style="font-size: 0.9rem; color: #d32f2f; font-weight: bold;">Faturadas ao término</div><ul class="lista-itens">{html_itens}</ul></div>', unsafe_allow_html=True)
