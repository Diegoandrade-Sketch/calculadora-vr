...
APP_VERSION = "v1.8.0 - Enterprise Edition (RBAC & VIP Sorting)"
...
# ==========================================
# DATA LAYER (SISTEMAS E PRODUTOS)
# ==========================================
...
def carregar_dados_vendas():
    ...
    try:
        ...
        if os.path.exists(CACHE_FILE):
            try:
                ...
                status_msg, status_cor = "Modo Offline (Cache)", "#facc15"
            except Exception:
    ...
    try:
        ...
        return {}, {}, {}, {}, {}, {}, {}, "Erro Processamento", "#ef4444", pd.DataFrame(), pd.DataFrame()
...
# ==========================================
# ESTADO GLOBAL DO APP E AUTENTICACAO
# ==========================================
...
# ==========================================
# BLOCO 1: MODULO DE SEGURANCA (LOGIN)
# ==========================================
def tela_login():
    if os.path.exists("logo_vr.png"):
        c1, c2, c3 = st.columns([1,2,1])
        with c2: st.image("logo_vr.png", use_container_width=True)
    
    st.markdown("<div class='login-box'>", unsafe_allow_html=True)
    st.markdown("<h3 style='text-align:center; color:#262730; margin-bottom:20px;'>Acesso à Plataforma</h3>", unsafe_allow_html=True)
    
    if st.session_state.primeiro_acesso:
        st.warning("Primeiro Acesso Detectado! Defina sua senha pessoal e intransferível.")
        ...
        # Bypass Administrativo de Emergencia (Para garantir que voce nao fique de fora se o banco falhar)
        ...
        if st.button("Autenticar", use_container_width=True):
            if not CONN_STR:
                st.error("Conexão com o banco de dados falhou. Use o acesso de emergência ou contate o suporte.")
            else:
                try:
                    engine = create_engine(CONN_STR)
...
def aplicativo_principal():
    ...
    # SIDEBAR COM CONTROLE RBAC (Visao Vendedor vs Admin)
    with st.sidebar:
        if os.path.exists("logo_vr.png"): st.image("logo_vr.png", width=180)
        st.markdown(f"<div style='font-size:0.85rem; font-weight:bold; color:#777; margin-bottom:15px;'>👤 {st.session_state.user_name}</div>", unsafe_allow_html=True)
    ...
    if tela == "Painel Admin":
        ...
        if "Offline" in db_status:
            st.error("Voce esta no Modo Offline. O Painel Admin esta bloqueado.")
        else:
                ...
                with st.form("form_v"):
                    ...
                    if st.form_submit_button("Salvar Vinculo"):
                        try:
                            ...
                            st.success("Vinculo Criado!"); st.cache_data.clear()
    ...
    # TELA 2: GERADOR DE PROPOSTA
        ...
        def aplicar_mapeamento():
            ...
            st.session_state.ui_sel_m, st.session_state.sel_m = _sel_m, _sel_m
            st.session_state.ui_sel_i, st.session_state.sel_i = _sel_i, _sel_i
            st.session_state.ui_sel_d, st.session_state.sel_d = _sel_d, _sel_d
            processar_regras_colaterais()
        ...
        # ==================== VIP LIST E ORDENACAO ====================
        def sort_mensal(item):
            if item == "VR ERP PRO": return 1
            if item == "VR PDV Convencional": return 2
            if "VR Sitef Express" in item: return 3
            if item == "VR Gerenciador Xml": return 4
            if "VR Mobile" in item: return 5
            return 999
        ...
        for n in st.session_state.sel_i:
            ...
            if q > 0:
                ...
                setup_lines.append({'nome': n, 'html': f"<li><span>{n}</span><span class='item-detalhe'>{int(q)}h x R$ {f_br(v_u)} | Total: R$ {f_br(q*v_u)}</span></li>", 'val': q*v_u})
        ...
        for n in st.session_state.sel_m:
            if name_to_id.get(n) not in vinculos_db:
                ...
                if h > 0:
                    ...
                    # Tratamento Exato da Nomenclatura do ERP
                    nome_setup = "Projeto ERP PRO" if n == "VR ERP PRO" else f"Implantacao {n}"
                    setup_lines.append({'nome': nome_setup, 'html': f"<li><span>{nome_setup}</span><span class='item-detalhe'>{int(h)}h x R$ {f_br(v_rate)} | Total: R$ {f_br(h*v_rate)}</span></li>", 'val': h*v_rate})
                if ads > 0:
                    setup_lines.append({'nome': f"Taxa de Adesao {n}", 'html': f"<li><span>Taxa de Adesao {n}</span><span class='item-detalhe'>1 un x R$ {f_br(ads)} | Total: R$ {f_br(ads)}</span></li>", 'val': ads})

        setup_lines.sort(key=sort_setup)
        t_setup = sum(i['val'] for i in setup_lines)
        h_setup = "".join(i['html'] for i in setup_lines)
        ...
        t_mensal, h_m = 0.0, ""
        for n in sorted(st.session_state.sel_m, key=sort_mensal):
            q = st.session_state[f"perm_val_{n}"]
            if q > 0:
        ...
        if exibir_media_loja:
            ...
            if qtd_lojas > 0:
                ...
                m_setup, m_mensal, m_desp = t_setup / qtd_lojas, t_mensal / qtd_lojas, (t_d if 't_d' in locals() else 0.0) / qtd_lojas
                m_cols = st.columns(3) if perfil_venda == "Executivo (Rua)" else st.columns([1, 2, 2, 1])[1:3]
                with m_cols[0]: st.markdown(f"""<div style="background-color:#ffffff; border-left: 6px solid #ff6600; padding:15px; border-radius:5px; box-shadow: 0 2px 5px rgba(0,0,0,0.05);"><span style="font-size:0.85rem; font-weight:bold; color:#777;">SETUP POR LOJA</span><br><span style="font-size:1.6rem; font-weight:900; color:#333;">R$ {f_br(m_setup)}</span></div>""", unsafe_allow_html=True)
                with m_cols[1]: st.markdown(f"""<div style="background-color:#ffffff; border-left: 6px solid #2e7d32; padding:15px; border-radius:5px; box-shadow: 0 2px 5px rgba(0,0,0,0.05);"><span style="font-size:0.85rem; font-weight:bold; color:#777;">MENSALIDADE POR LOJA</span><br><span style="font-size:1.6rem; font-weight:900; color:#333;">R$ {f_br(m_mensal)}</span></div>""", unsafe_allow_html=True)
                if perfil_venda == "Executivo (Rua)":
                    with m_cols[2]: st.markdown(f"""<div style="background-color:#ffffff; border-left: 6px solid #1976d2; padding:15px; border-radius:5px; box-shadow: 0 2px 5px rgba(0,0,0,0.05);"><span style="font-size:0.85rem; font-weight:bold; color:#777;">DESPESAS POR LOJA</span><br><span style="font-size:1.6rem; font-weight:900; color:#333;">R$ {f_br(m_desp)}</span></div>""", unsafe_allow_html=True)
            else: st.warning("Para exibir a media por loja, e necessario incluir pelo menos 1 unidade do VR ERP PRO.")
    ...
    # TELA 3: CONSULTA DE PRECO
        ...
        if p_sel:
            ...
            if is_sistema:
                ...
                with c3: st.markdown(f"""<div class="resumo-card" style="border-top-color:#262730; min-height: auto;"><span>Resumo Anual</span><div style="margin-top:15px;"><p><b>Economia Mensal:</b> R$ {f_br(v_b-v_l)}</p><p><b>Economia Anual:</b> R$ {f_br((v_b-v_l)*12)}</p></div></div>""", unsafe_allow_html=True)
            else:
                c1, c2 = st.columns(2)
                with c1:
                    html_b = f"""<span style="text-decoration: line-through; color: #777; font-size: 0.9rem;">R$ {f_br(v_b)}</span>""" if desc_s > 0 else ""
                ...
                with c2: st.markdown(f"""<div class="resumo-card" style="border-top-color:#262730; min-height: auto;"><span>Resumo do Desconto</span><div style="margin-top:15px;"><p><b>Economia Total Gerada:</b> R$ {f_br(v_b-v_l)}</p><p style="color:#777; font-size:0.85rem;">*Este item nao possui faturamento recorrente mensal.</p></div></div>""", unsafe_allow_html=True)

# ==========================================
# BLOCO 3: ROTEADOR DE ACESSO
# ==========================================
