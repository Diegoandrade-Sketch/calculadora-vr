...
if 'ms_sel_i' not in st.session_state: st.session_state.ms_sel_i = []
if 'ms_sel_m' not in st.session_state: st.session_state.ms_sel_m = []
...
def limpar_tudo():
    for k in map_keys:
        st.session_state[k] = 0 if any(x in k for x in ['pdv', 'semanas']) else "Não utiliza" if 'tef' in k else False
    st.session_state.ms_sel_i = []
    st.session_state.ms_sel_m = []
    for nome in full_db.keys():
        st.session_state[f"perm_val_{nome}"] = 0

# --- PARTE 1: MAPEAMENTO DA OPERAÇÃO (LÓGICA) ---
def aplicar_mapeamento():
    # Trabalhamos com cópias das listas para o Streamlit detectar a mudança
    nova_sel_m = list(st.session_state.ms_sel_m)
    nova_sel_i = list(st.session_state.ms_sel_i)
    ...
    for p, qtd in pdv_map.items():
        if p in sistemas_db:
            ...
            if qtd > 0 and p not in nova_sel_m: nova_sel_m.append(p)
    
    # TEF
    total = sum(pdv_map.values())
    nova_sel_m = [i for i in nova_sel_m if "SiTef" not in i] # Reset do TEF anterior
    ...
    if it in servicos_db:
        ...
        if sem > 0 and it not in nova_sel_i: nova_sel_i.append(it)
    ...
    for item, ativo in exp.items():
        if item in sistemas_db:
            ...
            if ativo and item not in nova_sel_m: nova_sel_m.append(item)

    # Atualiza o estado dos widgets
    st.session_state.ms_sel_m = nova_sel_m
    st.session_state.ms_sel_i = nova_sel_i
...
if tela == "Gerador de Proposta":
    if not modo_apresentacao:
        ...
        # PARTE 2: TELA DE VENDA (INCLUSÃO MANUAL - TOTALMENTE LIBERADA)
        ...
        with col_i:
            ...
            # O multiselect agora usa a chave ms_sel_i para persistir o estado manual
            lista_i = st.multiselect("Serviços", list(servicos_db.keys()), key="ms_sel_i")
            for i in lista_i:
        ...
        with col_m:
            ...
            lista_m = st.multiselect("Sistemas", list(sistemas_db.keys()), key="ms_sel_m")
            for i in lista_m:
    ...
    t_imp = sum(st.session_state[f"perm_val_{i}"] * servicos_db.get(i, {"Valor": 0})["Valor"] for i in st.session_state.ms_sel_i)
    t_men_bruto = sum(st.session_state[f"perm_val_{i}"] * sistemas_db.get(i, {"Valor": 0})["Valor"] for i in st.session_state.ms_sel_m)
    ...
    with res_cols[0]:
        html_i = "".join([f"<li><span>{i}</span><span class='item-detalhe'>{st.session_state[f'perm_val_{i}']}h x R$ {servicos_db.get(i, {'Valor':0})['Valor']:,.2f}</span></li>" for i in st.session_state.ms_sel_i if i in servicos_db])
    ...
    with res_cols[1]:
        html_m = "".join([f"<li><span>{i}</span><span class='item-detalhe'>{st.session_state[f'perm_val_{i}']} un x R$ {sistemas_db.get(i, {'Valor':0})['Valor']:,.2f}</span></li>" for i in st.session_state.ms_sel_m if i in sistemas_db])
        ...
        st.markdown(f'<div class="resumo-card" style="border-top-color: #2e7d32;"><span class="resumo-label">Mensalidade</span><div class="resumo-valor" style="color: #2e7d32;">R$ {t_men_liq:,.2f}</div>{desc_txt}<div class="resumo-subtitulo">SISTEMAS</div><ul class="lista-itens">{html_m if html_m else "<li>Nenhum item</li>"}</ul></div>', unsafe_allow_html=True)
    ...
    if full_db:
        ...
        d = full_db[prod_sel]
        st.markdown(f'<div class="resumo-card" style="min-height:auto;"><span class="resumo-label">Valor Unitário</span><div class="resumo-valor">R$ {d["Valor"]:,.2f}</div><p><b>Categoria:</b> {d["Tipo_Busca"].upper()}</p><hr><p>{d.get("Descricao", "Sem descrição disponível.")}</p></div>', unsafe_allow_html=True)
```
