...
# Chaves para os Multiselects (Parte 2: Tela de Venda)
...
# Chaves para o Mapeamento (Parte 1)
...
# Valores persistentes de quantidades/horas
...
# --- LÓGICA DE AUTOMATIZAÇÃO (AUXILIA A PARTE 2) ---
def aplicar_mapeamento():
    ...
    nova_sel_m = [i for i in nova_sel_m if "SiTef" not in i]
...
if tela == "Gerador de Proposta":
    ...
    with col_i:
        ...
        itens_escolhidos_i = st.multiselect("Serviços", list(servicos_db.keys()), key="ms_sel_i")
        for i in itens_escolhidos_i:
    ...
    with col_m:
        ...
        itens_escolhidos_m = st.multiselect("Sistemas", list(sistemas_db.keys()), key="ms_sel_m")
        for i in itens_escolhidos_m:
    ...
    # PARTE 3: CARDS DE RESUMO (RESULTADOS)
    ...
    if full_db:
        ...
        d = full_db[prod_sel]
        st.markdown(f'<div class="resumo-card" style="min-height:auto;"><span class="resumo-label">Valor Unitário</span><div class="resumo-valor">R$ {d["Valor"]:,.2f}</div><hr><p>{d.get("Descricao", "Sem descrição disponível.")}</p></div>', unsafe_allow_html=True)
