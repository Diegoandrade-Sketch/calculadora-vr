...
def tela_visao_comercial():
    import datetime # Garante que o módulo está disponível localmente
    import pandas as pd
    ...
    try:
        # Trazendo os inputs de data para DENTRO da proteção contra falhas
...
def aplicativo_principal():
   ...
   # ==========================================
    ...
    # ==========================================
        ...
        # 1. Buscar Negócios Ganhos Recentes
        try:
            engine = get_db_engine()
            with engine.connect() as conn:
                query_negocios = text("""
        ...
        if df_negocios.empty:
            ...
            if negocio_selecionado != "Selecione um contrato...":
                ...
                # 2. Puxar Dados Estruturais do Negócio Direto do Banco (Já com JOIN na tabela product)
                with engine.connect() as conn:
                    query_cnpjs = text("""
                    ...
                    # A mágica acontece aqui: O banco já traz as porcentagens prontas cruzando o ufcrmCodigo com o id do product
                    query_itens = text("""
                        SELECT 
                            io.id, 
                            io.title, 
                ...
                if df_itens.empty:
                    ...
                    # 3. Cálculo Inteligente (Sem dependência de arquivos soltos)
                    for _, item in df_itens.iterrows():
                        ...
                        # Transforma os percentuais do banco (ex: 90.0) em decimais (0.90)
