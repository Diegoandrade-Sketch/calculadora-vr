    ...
    if full_db:
        ...
        if p_sel:
            ...
            with c2:
                ...
                bloco_mensal = f'<div class="resumo-subtitulo">DETALHE</div><ul class="lista-itens"><li><span>Desconto Aplicado</span><span class="item-detalhe">{desc_simulacao}% off</span></li><li><span>Valor Original</span><span class="item-detalhe">R$ {f_br(v_mensal_bruto)}</span></li></ul>' if desc_simulacao > 0 else ""
            ...
            with c3:
                st.markdown(f'<div class="resumo-card" style="border-top-color:#262730; min-height: auto;"><span class="resumo-label">Resumo da Negociação</span><div style="margin-top:15px; font-size:0.95rem; color:#444;"><p><b>Desconto:</b> {desc_simulacao}%</p><p><b>Economia Mensal:</b> R$ {f_br(v_mensal_bruto - v_mensal_liq)}</p><p><b>Economia Anual:</b> R$ {f_br((v_mensal_bruto - v_mensal_liq)*12)}</p><hr><p style="font-size: 0.85rem; color: #666;"><i>Interface otimizada para negociações individuais.</i></p></div></div>', unsafe_allow_html=True)
