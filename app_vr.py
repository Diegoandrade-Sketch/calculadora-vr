...
st.markdown("""
    ...
    div.stButton > button { width: 100%; background: linear-gradient(90deg, #ff6600 0%, #ff944d 100%); color: white; border: none; padding: 12px 20px; border-radius: 8px; font-weight: bold; font-size: 1rem; transition: all 0.3s ease; box-shadow: 0 4px 15px rgba(255, 102, 0, 0.2); }
    div.stButton > button:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(255, 102, 0, 0.4); color: white; }
...
# --- 3. BARRA LATERAL (TRAVAS DE NEGÓCIO) ---
with st.sidebar:
...
# 5. Dados Base (FIXOS POR ENQUANTO)
itens_imp = {
...
descricoes_imp = {
    ...
    "Configuração Servidor / PDV Linux": "Preparação técnica do servidor e dos terminais com Linux.",
...
with res_col2:
    html_itens = "".join([f"<li><span>{i}</span><span class='item-detalhe'>{q} Lic.</span></li>" for i, q, v in dados_mensal_final])
    cor_txt = "#d32f2f" if desc > 15.0 else "#2e7d32"
    st.markdown(f'<div class="resumo-card" style="border-top-color: {cor_txt};"><span class="resumo-label">Mensal</span><div class="resumo-valor">R$ {t_men_liq:,.2f}</div><div style="color:{cor_txt}; font-weight:bold;">Desconto: {desc}%</div><div class="resumo-subtitulo">SISTEMAS</div><ul class="lista-itens">{html_itens}</ul></div>', unsafe_allow_html=True)

with res_col3:
    html_itens = "".join([f"<li><span>{i}</span><span class='item-detalhe'>{q}</span></li>" for i, q, v in dados_desp_final])
    st.markdown(f'<div class="resumo-card" style="border-top-color: #1976d2;"><span class="resumo-label">Despesas</span><div class="resumo-valor">R$ {t_desp:,.2f}</div><div style="font-size:0.85rem; background:#fff5f5; color:#d32f2f; padding:5px; border-radius:4px;">Faturadas ao término</div><div class="resumo-subtitulo">LOGÍSTICA</div><ul class="lista-itens">{html_itens}</ul></div>', unsafe_allow_html=True)
