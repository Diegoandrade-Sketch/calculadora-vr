# ==========================================
# GESTÃO COMERCIAL E FINANCEIRA (MÓDULOS ADMIN)
# ==========================================
...
def tela_visao_comercial():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            with col_kpi1: st.markdown(f"""<div class="dash-card" style="border-top: 5px solid #1976d2; min-height:auto;"><div class="dash-title">Total de MRR (Mensalidade)</div><div style="font-size:1.8rem; font-weight:900; color:#262730;">R$ {f_br(t_mrr)}</div></div>""", unsafe_allow_html=True)
            with col_kpi2: st.markdown(f"""<div class="dash-card" style="border-top: 5px solid #ff6600; min-height:auto;"><div class="dash-title">Total de Setup (Serviços)</div><div style="font-size:1.8rem; font-weight:900; color:#262730;">R$ {f_br(t_setup)}</div></div>""", unsafe_allow_html=True)
            with col_kpi3: st.markdown(f"""<div class="dash-card" style="border-top: 5px solid #2e7d32; min-height:auto; background:#f4f6f9;"><div class="dash-title">Volume Total Fechado</div><div style="font-size:1.8rem; font-weight:900; color:#262730;">R$ {f_br(t_mrr + t_setup)}</div></div>""", unsafe_allow_html=True)
...
def aplicativo_principal():
    ...
    if menu == "Mapeamento Operacional":
        st.info("Ambiente de Mapeamento Operacional (Sua função original carrega aqui)")
        # tela_mapeamento_operacional() # Descomente quando for plugar a função original
    elif menu == "Proposta Comercial":
        st.info("Ambiente de Proposta Comercial (Sua função original carrega aqui)")
        # tela_proposta_comercial() # Descomente quando for plugar a função original
    elif menu == "Resumo de Investimentos":
        st.info("Ambiente de Resumo de Investimentos (Sua função original carrega aqui)")
        # tela_resumo_investimentos() # Descomente quando for plugar a função original
