...
# 3. Base de Dados
precos_tabela = {
    "VR ERP PRO": {"setup": 2415.60, "mensal": 1285.71, "cac": 3000.00, "margem": "Alta", "comp": "Alta", "desc": "Sistema de gestão completo.", "roi": "Redução de 15% em perdas."},
    "VR PDV Convencional": {"setup": 201.30, "mensal": 185.71, "cac": 400.00, "margem": "Média", "comp": "Média", "desc": "Frente de caixa estável.", "roi": "Agilidade no atendimento."},
    "PDV Touchscreen": {"setup": 201.30, "mensal": 185.71, "cac": 400.00, "margem": "Média", "comp": "Média", "desc": "Interface moderna.", "roi": "Treinamento simplificado."},
    "PDV Selfcheckout": {"setup": 500.00, "mensal": 290.44, "cac": 800.00, "margem": "Alta", "comp": "Alta", "desc": "Autoatendimento.", "roi": "Redução de custos fixos."},
    "SiTef Express": {"setup": 0.00, "mensal": 357.14, "cac": 100.00, "margem": "Altíssima", "comp": "Baixa", "desc": "Pagamentos em nuvem.", "roi": "Segurança transacional."},
    "VR TEF": {"setup": 0.00, "mensal": 417.04, "cac": 150.00, "margem": "Altíssima", "comp": "Baixa", "desc": "Transferência Eletrônica.", "roi": "Conciliação automática."},
    "Gerenciador XML": {"setup": 0.00, "mensal": 163.84, "cac": 50.00, "margem": "Alta", "comp": "Baixa", "desc": "Gestão de Notas.", "roi": "Automação fiscal."},
    "VR Mobile": {"setup": 201.30, "mensal": 193.63, "cac": 300.00, "margem": "Média", "comp": "Média", "desc": "Gestão móvel.", "roi": "Decisões em tempo real."},
    "Migração Banco de Dados": {"setup": 201.30, "mensal": 0.00, "cac": 200.00, "margem": "Baixa", "comp": "Alta", "desc": "Cópia de cadastro.", "roi": "Transição segura."},
    "Definição de Escopo": {"setup": 201.30, "mensal": 0.00, "cac": 100.00, "margem": "Média", "comp": "Média", "desc": "Mapeamento.", "roi": "Projeto assertivo."},
    "Configuração Servidor / PDV Linux": {"setup": 201.30, "mensal": 0.00, "cac": 150.00, "margem": "Média", "comp": "Média", "desc": "Ambiente Linux.", "roi": "Estabilidade total."},
    "Implantação e Treinamento": {"setup": 201.30, "mensal": 0.00, "cac": 1000.00, "margem": "Baixa", "comp": "Alta", "desc": "Capacitação.", "roi": "Equipe produtiva."}
}
...
if tela == "Consulta de Preço":
    ...
    with c2:
        st.markdown('<div class="section-header"><span class="section-title">DETALHES</span></div>', unsafe_allow_html=True)
...
# --- 6. TELA DE PROPOSTA ---
    ...
    if not modo_apresentacao:
        ...
        with col_i:
            ...
            for i in st.session_state.sel_i:
                ...
                st.number_input(f"H: {i}", min_value=0, value=12, key=f"v_h_{i}")
        ...
        with col_m:
            ...
            for i in st.session_state.sel_m:
                ...
                st.number_input(f"Q: {i}", min_value=0, value=1, key=f"v_q_{i}")
    ...
    for i in st.session_state.sel_i:
        vu = precos_tabela[i]["setup"]; h = st.session_state.get(f"v_h_{i}", 12)
        t_imp += h * vu; lista_i.append((i, h, vu))

    for i in st.session_state.sel_m:
        vu = precos_tabela[i]["mensal"]; q = st.session_state.get(f"v_q_{i}", 1)
        t_men_bruto += q * vu; lista_m.append((i, q, vu))
    ...
    st.markdown("<h2 style='text-align:center; font-weight:800; margin-top:30px;'>RESUMO FINAL</h2>", unsafe_allow_html=True)
    res_cols = st.columns(3) if perfil_venda == "Executivo (Rua)" else st.columns([1, 2, 2, 1])[1:3]

    with res_cols[0]:
        st.markdown(f'<div class="resumo-card"><span class="resumo-label">Setup</span><div class="resumo-valor">R$ {t_imp:,.2f}</div><div>{parcelas}x R$ {t_imp/parcelas:,.2f}</div></div>', unsafe_allow_html=True)

    with res_cols[1]:
        st.markdown(f'<div class="resumo-card" style="border-top-color: #2e7d32;"><span class="resumo-label">Mensalidade</span><div class="resumo-valor" style="color: #2e7d32;">R$ {t_men_liq:,.2f}</div><div>Faturamento: {faturamento}</div></div>', unsafe_allow_html=True)

    if perfil_venda == "Executivo (Rua)":
        with res_cols[2]:
            st.markdown(f'<div class="resumo-card" style="border-top-color: #1976d2;"><span class="resumo-label">Despesas</span><div class="resumo-valor" style="color: #1976d2;">R$ {t_desp:,.2f}</div></div>', unsafe_allow_html=True)
